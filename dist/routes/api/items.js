...
router.post('/', function (req, res) {
    var newItem = new Item_1.default({
        name: req.body.name
    });
...
router.delete('/:id', function (req, res) {
    Item_1.default.findById(req.params.id)
        ...
        .then(function (item) { return item.remove()
        .then(function () { return res.json({ message: 'Item deleted.' })
        .catch(function (error) { return res.status(404).json({ message: error.message }); }); }); });
});
